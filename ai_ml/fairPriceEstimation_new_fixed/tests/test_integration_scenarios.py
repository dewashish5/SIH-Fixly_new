...
def test_scenario_standard_afternoon_plumber(engine):
    # Customer books standard scheduled plumber on weekday afternoon in Bengaluru (X-tier)
    req = PriceEstimationRequest(
        service_category="plumbing",
        sub_service="tap_and_pipe_repair",
        location_pincode="560038",  # X tier: 1.30x
    ...
    assert resp.breakdown.city_tier == "X"
    # Base 171.53 * 1.30 = ~223
    assert 200.0 <= resp.estimated_price <= 300.0
...
def test_scenario_peak_evening_ac_repair(engine):
    ...
    req = PriceEstimationRequest(
        ...
        location_pincode="560034",  # Koramangala high density, X tier
    ...
    assert resp.estimated_price > 350.0
...
def test_scenario_midnight_emergency_short_circuit(engine):
    ...
    req = PriceEstimationRequest(
        ...
        location_pincode="560100",  # Electronic City, X tier
    ...
    assert resp.estimated_price >= 500.0

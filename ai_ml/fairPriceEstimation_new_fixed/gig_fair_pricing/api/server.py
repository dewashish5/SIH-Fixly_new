"""
Standalone Lightweight HTTP Server with CORS and Static File Hosting for Fair Pricing.
"""
...
class FairPricingHTTPRequestHandler(BaseHTTPRequestHandler):
...
def create_server(host: str = "127.0.0.1", port: int = 8081, engine: Optional[FairPriceEngine] = None) -> HTTPServer:
    FairPricingHTTPRequestHandler.router = APIRouter(engine or FairPriceEngine())
    server = HTTPServer((host, port), FairPricingHTTPRequestHandler)
    return server
...
def run_server(host: str = "127.0.0.1", port: int = 8081):
    ...
    print("=========================================================")
    print(f"[FAIR PRICING ENGINE] Server is RUNNING at:")
    print(f"URL: http://{host}:{port}/")
    print(f"Interactive Customer Simulator: http://{host}:{port}/demo/index.html")
    print(f"API Health Check:               http://{host}:{port}/api/health")
    print(f"Available Services:             http://{host}:{port}/api/services")
    print("=========================================================")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fair Price Estimation Standalone Server")
